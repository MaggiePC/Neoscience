...
const startGame = (canvas: HTMLCanvasElement, navigate: (route: string) => void) => {
    ...
    // 🟢 Inicialización y comprobación de 'ctx'
    ...
    function update(){
      ...
      if (!state.over){
        for (let r of state.rocks){ r.y += r.vy; r.x += r.vx; r.rot += r.rotSpeed; }
      }
      ...
      for (let i=state.rocks.length-1;i>=0;i--){
        ...
        for (let j=state.bullets.length-1;j>=0;j--){
          ...
          if (dx*dx + dy*dy < (r.r*0.8)*(r.r*0.8)){
            hitRock(r); state.rocks.splice(i,1); state.bullets.splice(j,1);
            state.score += 10; updateHUD(); break;
          }
        }
      }
    ...
    function hitRock(r: any){
      for (let k=0;k<16;k++){
        const p=particle(r.x,r.y,1); p.vx*=1.5; p.vy*=1.5; p.life=26; state.particles.push(p);
      }
    }
    ...
    function damageEarth(){
      ...
      if (state.lives<=0){
        state.over=true;
        if (state.score>best){ best=state.score; storage.setItem(BEST_KEY,String(best)); showBanner(`🎉 ¡Nuevo récord! ${best} puntos`); }
    ...
    // Dibujo
    function draw(){
      // 🟢 CORRECCIÓN: Uso del operador de aserción no nula '!'
...
export default function Game() {
 ...
 const { t, i18n } = useTranslation();
  // Inyectar los estilos CSS en el head del documento
  useMemo(() => {
  ...
  // useEffect para inicializar y limpiar el juego de Canvas
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden' }}>
        ...
        <div className="touch" id="touchCtrls" style={{ display: 'none' }}>
          ...
          <div className="intro-card">
            ...
            <div className="intro-grid">
              <section className="intro-section">
                ...
                <p style={{ margin: '6px 0 0', fontSize: '14px', lineHeight: 1.55 }}> La misión <b>DART</b> utilizó la técnica del <b>impacto cinético </b>, que consiste en estrellar intencionalmente una nave espacial contra un asteroide para alterar ligeramente su velocidad y, con el tiempo, su trayectoria.</p> <p style={{ margin: '6px 0 0', fontSize: '14px', lineHeight: 1.55 }}> <b>Funcionó</b>: la trayectoria se desvió, mostrando que, con tiempo, podemos evitar que una roca peligrosa nos visite. 🌍🚀 </p> <p style={{ margin: '6px 0 0', fontSize: '14px', lineHeight: 1.55 }}> Mientras antes actúes, más fácil es desviar el asteroide </p>
